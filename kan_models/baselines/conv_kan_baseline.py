...
from kan_convs import KANConv2DLayer, KANConv2DLayerMBN
...
class SimpleConvKANMBN(nn.Module):
    def __init__(
            self,
            layer_sizes,
            num_classes: int = 10,
        ...
        super(SimpleConvKANMBN, self).__init__()

        self.layers = nn.Sequential(
            KANConv2DLayerMBN(input_channels, layer_sizes[0], kernel_size=3, spline_order=spline_order, groups=1,
                           padding=1, stride=1, dilation=1, affine=affine, bn_types=bn_types),
            L1(KANConv2DLayerMBN(layer_sizes[0], layer_sizes[1], kernel_size=3, spline_order=spline_order, groups=groups,
                              padding=1, stride=2, dilation=1, dropout=dropout, affine=affine, bn_types=bn_types),
               l1_penalty),
            L1(KANConv2DLayerMBN(layer_sizes[1], layer_sizes[2], kernel_size=3, spline_order=spline_order, groups=groups,
                              padding=1, stride=2, dilation=1, dropout=dropout, affine=affine, bn_types=bn_types),
               l1_penalty),
            L1(KANConv2DLayerMBN(layer_sizes[2], layer_sizes[3], kernel_size=3, spline_order=spline_order, groups=groups,
                              padding=1, stride=1, dilation=1, dropout=dropout, affine=affine, bn_types=bn_types),
               l1_penalty),
            nn.AdaptiveAvgPool2d((1, 1))
        )
        if degree_out < 2:
...
class EightSimpleConvKANMBN(nn.Module):
    def __init__(
            self,
            layer_sizes,
            num_classes: int = 10,
        ...
        super(EightSimpleConvKANMBN, self).__init__()
        ...
        self.layers = nn.Sequential(
            ...
            L1(KANConv2DLayerMBN(layer_sizes[2], layer_sizes[3], kernel_size=3, spline_order=spline_order, groups=groups,
                              padding=1, stride=1, dilation=1, dropout=dropout, affine=affine, bn_types=bn_types),
               l1_penalty),
            L1(KANConv2DLayerMBN(layer_sizes[3], layer_sizes[4], kernel_size=3, spline_order=spline_order, groups=groups,
                              padding=1, stride=1, dilation=1, dropout=dropout, affine=affine, bn_types=bn_types),
               l1_penalty),
            L1(KANConv2DLayerMBN(layer_sizes[4], layer_sizes[5], kernel_size=3, spline_order=spline_order, groups=groups,
                              padding=1, stride=2, dilation=1, dropout=dropout, affine=affine, bn_types=bn_types),
               l1_penalty),
            L1(KANConv2DLayerMBN(layer_sizes[5], layer_sizes[6], kernel_size=3, spline_order=spline_order, groups=groups,
                              padding=1, stride=1, dilation=1, dropout=dropout, affine=affine, bn_types=bn_types),
               l1_penalty),
            L1(KANConv2DLayerMBN(layer_sizes[6], layer_sizes[7], kernel_size=3, spline_order=spline_order, groups=groups,
                              padding=1, stride=1, dilation=1, dropout=dropout, affine=affine, bn_types=bn_types),
               l1_penalty),
            nn.AdaptiveAvgPool2d((1, 1))
        )
