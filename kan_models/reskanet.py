...
class ResKANetMBN(nn.Module):
    def __init__(
            self,
            block: Type[Union[BottleneckKAGNBasicBlockMBN]],
            layers: List[int],
            input_channels: int = 3,
            use_first_maxpool: bool = True,
            mp_kernel_size: int = 3, mp_stride: int = 2, mp_padding: int = 1,
        ...
        self.layer1 = self._make_layer(block, 8 * width_scale, layers[0], bn_types=bn_types)
        self.layer2 = self._make_layer(block, 16 * width_scale, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0], bn_types=bn_types)
        self.layer3 = self._make_layer(block, 32 * width_scale, layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1], bn_types=bn_types)
        self.layer4 = self._make_layer(block, 64 * width_scale, layers[3], stride=2,
                                       dilate=replace_stride_with_dilation[2], bn_types=bn_types)
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.drop = nn.Dropout(p=dropout_linear)
        self.fc = nn.Linear(64 * width_scale * block.expansion if self.hidden_layer is None else hidden_layer_dim,
                            num_classes)
    ...
    def _make_layer(
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            ...
            downsample = conv1x1(self.inplanes, planes * block.expansion, stride=stride, bn_types=bn_types)

        layers = []
        layers.append(
            block(
                self.inplanes, planes, stride=stride, downsample=downsample, groups=self.groups,
                base_width=self.base_width, dilation=previous_dilation, bn_types=bn_types
            )
        )
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
    ...
    def forward(self, x: Tensor, t=None, **kwargs) -> Tensor:
        if t is not None:
            self._set_bn_type(t)
...
def reskagnetbnMBN_18x32p(input_channels, num_classes, groups: int = 1, degree: int = 3, width_scale: int = 1,
                       hidden_layer_dim=None, dropout: float = 0.0, l1_decay: float = 0.0,
                       dropout_linear: float = 0.25, affine: bool = False, bn_types=['base']):
    return ResKANetMBN(BottleneckKAGNBasicBlockMBN, [2, 2, 2, 2],
                    input_channels=input_channels,
                    use_first_maxpool=False,
                    fcnv_kernel_size=3, fcnv_stride=1, fcnv_padding=1,
                    num_classes=num_classes,
